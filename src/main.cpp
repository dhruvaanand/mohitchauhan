...
#define IS_SIMULATOR 0 // Set to 0 when uploading to real ESP32
...
void sceneBuildup(uint32_t ms) {
  ...
  for (uint8_t y = 0; y < kMatrixHeight; y++) {
    for (uint8_t x = 0; x < kMatrixWidth; x++) {
      ...
      if (combinedNoise > 140) {
        uint8_t br = qsub8(combinedNoise, 130);
  ...
  uint8_t centerY = 9;
  uint8_t bpmWidth = beatsin8(BPM, 4, 16);
  for (int x = 9 - (bpmWidth / 2); x <= 9 + (bpmWidth / 2); x++) {
    if (x >= 0 && x < kMatrixWidth) {
      uint8_t h = inoise8(x * 100, ms / 2) / 40;
      if (h == 0) h = 1;
...
void sceneTunnel(uint32_t ms) {
  ...
  for(uint8_t y=0; y<kMatrixHeight; y++) {
    for(uint8_t x=0; x<kMatrixWidth; x++) {
      ...
      uint8_t br = 0;
      if (dist < 1.2) {
        br = qadd8(20, (1.0 - dist) * 200);
        br = qadd8(br, (combined > 0.4 ? 200 : 0));
      }
      index += (1.0 - dist) * 100;
      leds[XY(x, y)] = ColorFromPalette(tunnelPalette, index, br);
      if (combined > 0.8) {
        leds[XY(x, y)] += CRGB(40, 20, 20);
      }
    }
  }
}
...
// ==========================================================
struct Pose { Point head, neck, belly, lArm, rArm, lLeg, rLeg; };
...
void drawDancer(uint32_t ms, CRGB c) {
  ...
  Pose p1 = poses[currentStep]; Pose p2 = poses[nextStep];
  Point head = lerpPoint(p1.head, p2.head, frac); Point neck = lerpPoint(p1.neck, p2.neck, frac); Point belly = lerpPoint(p1.belly, p2.belly, frac);
  Point lArm = lerpPoint(p1.lArm, p2.lArm, frac); Point rArm = lerpPoint(p1.rArm, p2.rArm, frac); Point lLeg = lerpPoint(p1.lLeg, p2.lLeg, frac); Point rLeg = lerpPoint(p1.rLeg, p2.rLeg, frac);
  int8_t bob = sin8(ms / 2) / 64 - 2; head.y += bob; neck.y += bob; belly.y += bob; lArm.y += bob; rArm.y += bob; lLeg.y += bob; rLeg.y += bob;
  if (currentStep == 5 || currentStep == 6 || currentStep == 9) {
    float angle = (ms % 600) * (2.0 * PI / 600.0); Point center = belly;
    head = rotatePoint(head, angle, center); neck = rotatePoint(neck, angle, center); lArm = rotatePoint(lArm, angle, center); rArm = rotatePoint(rArm, angle, center); lLeg = rotatePoint(lLeg, angle, center); rLeg = rotatePoint(rLeg, angle, center);
  }
  for(int dx=-1; dx<=1; dx++) for(int dy=-1; dy<=1; dy++) if(abs(dx)+abs(dy)<2) leds[XY(head.x+dx, head.y+dy)] = c;
  drawLine(neck, belly, c); drawLine(neck, lArm, c); drawLine(neck, rArm, c); drawLine(belly, lLeg, c); drawLine(belly, rLeg, c);
}
